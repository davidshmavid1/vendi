...
import { initiateBookingAction } from "@/server/actions/bookings";
...
export function BookingCheckout({
  ...
  const start = useCallback(async () => {
    setLoading(true);
    setError(undefined);
    const result = await initiateBookingAction(orgSlug, spaceId);
    setLoading(false);
    if (!result.ok) {
  ...
  if (!clientSecret) {
    return (
      <div className="flex flex-col gap-3">
        ...
        >
          {loading ? "Starting…" : `Reserve for $${(price / 100).toFixed(2)}`}
        </button>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      {success && (
        <p className="text-sm text-green-600 dark:text-green-400">
          Payment submitted. Your booking will update as soon as Stripe confirms it.
        </p>
      )}
